...
function zf_ValidateAndSubmit(){
	...
	function zf_ShowErrorMsg(uniqName){
		var fldLinkName, errElem;
		for( errInd = 0 ; errInd < zf_FieldArray.length ; errInd ++ ) {
			fldLinkName = zf_FieldArray[errInd];
			var hideId = (typeof zf_ErrorIdMap !== 'undefined' && zf_ErrorIdMap[fldLinkName]) ? zf_ErrorIdMap[fldLinkName] : (fldLinkName.split('_')[0]+"_error");
			errElem = document.getElementById(hideId);
			if(errElem){ errElem.style.display = 'none'; }
		}
		var linkName = (typeof zf_ErrorIdMap !== 'undefined' && zf_ErrorIdMap[uniqName]) ? zf_ErrorIdMap[uniqName] : (uniqName.split('_')[0]+"_error");
		errElem = document.getElementById(linkName);
